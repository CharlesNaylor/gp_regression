...
transformed data {
  ...
  matrix[B,A] x_scaled[N]; // Scale xs so betas wind up on unit scale
  real delta = 1e-9;
  vector[B] beta_mu_mu = rep_vector(0, B);
  for(n in 1:N) {
    t[n] = n;
    L_y_cov[n] = cholesky_decompose(y_cov[n]);
    x_scaled[n] = x[n] * 1e-2; // two orders of magnitude
  }
}
parameters {
  matrix[N,B] beta;
...
model {
  ...
  for(n in 1:N1)
    y[n] ~ multi_normal_cholesky(beta[n] * x_scaled[n], L_y_cov[n]);
}
generated quantities {
  vector[A] new_y[N2];
  for(n in 1:N2)
    new_y[n] = multi_normal_cholesky_rng(to_vector(beta[N1+n] * x_scaled[N1+n]),
                                        L_y_cov[N1+n]);
}
